resource "lxd_container" "heat" {
  ...
  provisioner "remote-exec" {
    inline = [
      "lxc exec infra -- mysql -e 'drop database heat' || true",
      "lxc exec infra -- puppet agent -t || true",
      "lxc exec puppet -- puppet cert clean heat.lxd || true",
      "lxc exec heat -- bash /root/cloud-archive.sh",
      "lxc exec heat -- bash /root/puppet-agent.sh",
    ]
  }
}
