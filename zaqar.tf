resource "lxd_container" "zaqar" {
  ...
  provisioner "remote-exec" {
    inline = [
      "lxc exec infra -- mysql -e 'drop database zaqar' || true",
      "lxc exec infra -- puppet agent -t || true",
      "lxc exec puppet -- puppet cert clean zaqar.lxd || true",
      "lxc exec zaqar -- bash /root/cloud-archive.sh",
      "lxc exec zaqar -- bash /root/puppet-agent.sh",
    ]
  }
}
