resource "lxd_container" "senlin" {
  ...
  provisioner "remote-exec" {
    inline = [
      "lxc exec infra -- mysql -e 'drop database senlin' || true",
      "lxc exec infra -- puppet agent -t || true",
      "lxc exec puppet -- puppet cert clean senlin.lxd || true",
      "lxc exec senlin -- bash /root/cloud-archive.sh",
      "lxc exec senlin -- bash /root/puppet-agent.sh",
    ]
  }
}
